...
async function main(): Promise<void> {
  if (!process.env.DATABASE_URL) {
    throw new Error('DATABASE_URL is required for CoreSlot key rotation projection');
  }
  ...
  try {
    await withProjectionAdvisoryLock(prisma, async () => {
      if (process.env.RESET_PROJECTION === 'true') {
        await resetCoreSlotKeyRotationProjection(
          prisma as unknown as ResetKeyRotationProjectionPrisma,
        );
      }
      ...
      await projectCoreSlotKeyRotationRange({
        prisma: prisma as unknown as CoreSlotKeyRotationProjectionPrisma,
        chainId,
        startHeight,
        endHeight,
      });
