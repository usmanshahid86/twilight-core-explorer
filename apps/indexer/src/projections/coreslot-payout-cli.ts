...
import {
  resetCoreSlotPayoutProjection,
  type ResetPayoutProjectionPrisma,
} from './reset-payout.js';
...
async function main(): Promise<void> {
  if (!process.env.DATABASE_URL) {
    throw new Error('DATABASE_URL is required for CoreSlot payout projection');
  }
  ...
  try {
    await withProjectionAdvisoryLock(prisma, async () => {
      if (process.env.RESET_PROJECTION === 'true') {
        await resetCoreSlotPayoutProjection(prisma as unknown as ResetPayoutProjectionPrisma);
      }
      ...
      await projectCoreSlotPayoutRange({
        prisma: prisma as unknown as CoreSlotPayoutProjectionPrisma,
        chainId,
        startHeight,
        endHeight,
      });
