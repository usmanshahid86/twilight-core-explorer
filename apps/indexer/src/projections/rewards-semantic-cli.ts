...
import {
  ...
  type RewardsSemanticProjectionPrisma,
} from './rewards-semantic.js';
import {
  resetRewardsProjections,
  type ResetRewardsProjectionPrisma,
} from './reset-rewards.js';
import { REWARDS_SEMANTIC_PROJECTION } from './types.js';
...
async function main(): Promise<void> {
  if (!process.env.DATABASE_URL) {
    throw new Error('DATABASE_URL is required for the rewards semantic projection');
  }
  ...
  try {
    await withProjectionAdvisoryLock(prisma, async () => {
      ...
      await projectRewardsSemanticRange({
        prisma: prisma as unknown as RewardsSemanticProjectionPrisma,
        chainId,
        startHeight,
        endHeight,
      });
